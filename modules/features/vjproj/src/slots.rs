...
pub fn real_tag(project: u8, visible: u8) -> Result<u8> {
    ...
    if !(1..=VISIBLE_TAGS).contains(&visible) {
...
pub fn project_for(real: u8) -> Option<u8> {
    (1..=NUM_PROJECTS).find(|&project| {
        VISIBLE_SLOTS
            .iter()
            .any(|&visible| real_tag(project, visible).is_ok_and(|tag| tag == real))
    })
}

pub fn next_occupied(active: u8, occupied: &[u8]) -> u8 {
    let busy = |project: u8| {
        occupied
            .iter()
            .any(|&real| project_for(real) == Some(project))
    };
    (1..NUM_PROJECTS)
        .map(|step| (active + step - 1) % NUM_PROJECTS + 1)
        .find(|&project| busy(project))
        .unwrap_or(active % NUM_PROJECTS + 1)
}

pub fn nth_open(position: u8, open: &[u8]) -> Option<u8> {
    open.get(usize::from(position).checked_sub(1)?).copied()
}
