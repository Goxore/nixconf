{self, ...}: {
  modules.nixos.desktop = {
    ...
    discordCss = pkgs.runCommand "discord-gruvbox-firefox.css" {} ''
      {
        echo '@-moz-document domain("discord.com") {'
        awk -f ${./_importantify.awk} ${self.lib.discordTheme.stylesheet pkgs}
        echo '}'
      } > $out
    '';
  ...
  in {
    programs.firefox = {
      ...
      policies.ExtensionSettings."firefox@equicord.org" = {
