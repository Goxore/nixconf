{
  ...
  stateDir = ".local/share/TelegramDesktop";
in {
  wrappers.telegram = {
    wlib,
    pkgs,
    ...
    upstreamPalette = inputs.telegram-gruvbox;
    ...
    themeFile =
      pkgs.runCommand "gruvbox.tdesktop-theme" {
        ...
        magick ${self.lib.wallpaper} -blur ${backgroundBlur} contents/background.png
        cd contents
        zip -q -X $out colors.tdesktop-theme background.png
      '';

    installTheme = ''
      themePath="$HOME/${stateDir}/${themeFile.name}"
