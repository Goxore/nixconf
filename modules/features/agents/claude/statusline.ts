...
const sidecar = (data: StatusInput): Sidecar => {
  const { context_window_size: size = 0, used_percentage, current_usage = {} } =
    data.context_window ?? {}
  const { five_hour = {}, seven_day = {} } = data.rate_limits ?? {}

  return {
    model: (data.model?.display_name ?? "").replace(/\s*\(.*?\)/g, "").trim(),
    contextUsed: totalTokens(current_usage),
    contextSize: size,
    contextShare: used_percentage ?? null,
    fiveHour: five_hour.used_percentage ?? null,
    sevenDay: seven_day.used_percentage ?? null,
    resetsAt: formatClock(five_hour.resets_at),
  }
}

const publish = (data: StatusInput): void => {
  const pid = process.env.VJAGENT_PID
  if (!pid) return

  try {
    const dir = join(process.env.XDG_RUNTIME_DIR || "/tmp", "vjproj", "status")
    const path = join(dir, `${pid}.json`)
    const tmp = `${path}.tmp`

    mkdirSync(dir, { recursive: true })
    writeFileSync(tmp, JSON.stringify(sidecar(data)))
    renameSync(tmp, path)
  } catch {}
}
