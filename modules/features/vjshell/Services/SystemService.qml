...
Singleton {
    ...
    readonly property bool active: PanelService.isOpen("processes")
    ...
    readonly property string gpuBusyPath: sensors.gpuBusy ?? ""
    readonly property string vramUsedPath: sensors.vramUsed ?? ""
    readonly property string vramTotalPath: sensors.vramTotal ?? ""
    readonly property string gpuTempPath: sensors.gpuTemp ?? ""
    readonly property string gpuPowerPath: sensors.gpuPower ?? ""
    readonly property string cpuTempPath: sensors.cpuTemp ?? ""
    readonly property var polled: [statFile, meminfoFile, uptimeFile, loadFile, cpuTempFile, gpuBusyFile, vramUsedFile, gpuTempFile, gpuPowerFile]
    ...
    function parseStat(text) {
        ...
        for (const line of text.split("\n")) {
            ...
            const fields = line.split(/\s+/).slice(1, 9).map(Number);
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                for (const line of text.split("\n")) {
                    ...
                    if (space > 0)
                        found[line.slice(0, space)] = line.slice(space + 1);
                }
                root.sensors = found;
            }
        }
