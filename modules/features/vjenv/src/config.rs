use crate::paths::{Dirs, expand_tilde};
...
impl Config {
    ...
    pub fn load(dirs: &Dirs) -> Result<Self> {
        ...
        Self::parse(&text, &dirs.home).with_context(|| format!("cannot parse {}", path.display()))
    }
...
mod tests {
    ...
    fn parses_roots_with_tilde_expanded() {
        ...
        assert_eq!(
            c.roots.containers,
            vec![PathBuf::from("/home/yurii/Projects")]
        );
    ...
    fn parses_identities() {
        ...
        assert_eq!(g.email.as_deref(), Some("goxore@example.com"));
    ...
    fn malformed_toml_reports_where() {
        ...
        assert!(
            msg.contains("line") || msg.contains("expected"),
            "unhelpful error: {msg}"
        );
    }
}
