...
mod tests {
    ...
    fn full() -> Identity {
        Identity {
            ...
            email: Some("vimjoyer@example.com".into()),
    ...
    fn every_managed_variable_is_always_accounted_for() {
        for identity in [Some(&full()), None] {
            ...
            let emitted: BTreeSet<&str> = ops.iter().map(EnvOp::name).collect();
    ...
    fn a_full_identity_populates_all_three_git_slots() {
        ...
        assert_eq!(
            value(&ops, "GIT_CONFIG_VALUE_1"),
            Some("vimjoyer@example.com")
        );
    ...
    fn jj_base_handles_empty_and_vjenv_only_values() {
        ...
        assert_eq!(
            jj_base(Some("/run/user/1000/vjenv/goxore.jj.toml"), rt),
            None
        );
