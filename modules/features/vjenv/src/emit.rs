...
impl EnvOp {
    ...
    pub fn list(name: impl Into<String>, entries: Vec<String>) -> Self {
        EnvOp::SetList(name.into(), entries)
    }

    pub fn name(&self) -> &str {
        match self {
            EnvOp::Set(n, _) | EnvOp::SetList(n, _) | EnvOp::Unset(n) => n,
        }
    }

    pub fn matches(&self, current: Option<&str>) -> bool {
        match self {
            EnvOp::Set(_, v) => current == Some(v.as_str()),
            EnvOp::SetList(_, entries) => current == Some(entries.join(":").as_str()),
            EnvOp::Unset(_) => current.is_none(),
        }
    }
}
...
pub fn render(ops: &[EnvOp], shell: Shell) -> String {
    ...
    for op in ops {
        match (shell, op) {
            ...
            (Shell::Fish, EnvOp::SetList(n, entries)) => {
                let joined: Vec<String> = entries.iter().map(|e| quote(e, shell)).collect();
                let _ = writeln!(out, "set -gx {n} {}", joined.join(" "));
            }
            ...
            (Shell::Posix, EnvOp::SetList(n, entries)) => {
                let _ = writeln!(out, "export {n}={}", quote(&entries.join(":"), shell));
            }
...
mod tests {
    ...
    #[test]
    fn fish_receives_a_path_as_a_list_and_posix_as_one_string() {
        let op = EnvOp::list("PATH", vec!["/a bin".into(), "/b".into()]);
        assert_eq!(
            render(std::slice::from_ref(&op), Shell::Fish),
            "set -gx PATH '/a bin' '/b'\n"
        );
        assert_eq!(render(&[op], Shell::Posix), "export PATH='/a bin:/b'\n");
    }

    #[test]
    fn an_empty_list_still_assigns() {
        let op = EnvOp::list("PATH", Vec::new());
        assert_eq!(
            render(std::slice::from_ref(&op), Shell::Fish),
            "set -gx PATH \n"
        );
        assert_eq!(render(&[op], Shell::Posix), "export PATH=''\n");
    }

    #[test]
    fn an_op_knows_whether_the_environment_already_agrees() {
        assert!(EnvOp::set("A", "1").matches(Some("1")));
        assert!(!EnvOp::set("A", "1").matches(Some("2")));
        assert!(!EnvOp::set("A", "1").matches(None));

        assert!(EnvOp::unset("A").matches(None));
        assert!(!EnvOp::unset("A").matches(Some("")));

        let list = EnvOp::list("PATH", vec!["/a".into(), "/b".into()]);
        assert!(list.matches(Some("/a:/b")));
        assert!(!list.matches(Some("/b:/a")));
    }
